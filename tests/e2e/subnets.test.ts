...
test.describe("/subnets sending data tests", () => {
...
test.describe("/subnets Error tests", () => {
    test("Checking error on binary type", async ({ page }) => {
        await binaryError(page, "/subnets");
        const content = await page
            .locator(".content-container .error-container")
            .textContent();
        expect(content).toBe(
            "Your data seems to be incorrect. The request cannot be processed.",
        );
    });
});
